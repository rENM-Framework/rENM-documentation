...
- **`rENM.analysis`** — added `find_boundary_trend_statistics()`, comparing trend behavior inside the GAP range against the surrounding buffer ring. `find_trend_percentages()` gained a `layer` argument and per-class area columns, and its percentages moved from a cell-count basis to an area basis, matching every other percentage the framework reports. Several hot-spot corrections: area is clipped to the GAP range portion within each state and weighted by the fraction of each cell inside it, and Hot Spot % is taken against a grid-measured range area, so it can no longer exceed 100. Also fixed `create_hot_spot_map()` aborting where a range edge coincides with a state line, and both state functions including states with no raster coverage. `create_state_trend_analysis()`'s positive and negative percentages are now taken over the range area carrying trend data rather than the whole range, so they sum to 100 and share a basis with the boundary rows printed in the same columns; a new `GAP.RANGE.DATA.PCT` column records the coverage. `find_boundary_trend_statistics()` also reports the median and median absolute slope per zone, because a percentage counts area above and below zero and a zone whose slopes hover near zero yields a figure near 50 that carries no signal.
- **`rENM.ai`** — added `assemble_coversheet()`, which builds the narrative page locally with no language model, used when `ai = NULL` and as the fallback when a provider call fails. Adds `officer` to Imports. `submit_to_chatgpt()`'s container listing is now paginated, which was the actual cause of document retrieval failing at random. `render_ai_docx()` runs four checks on the returned document: unsubstituted placeholders and any edit to the text supplied for copying stop the run, truncation and known prose faults warn. Every report's opening page now carries a REPRODUCIBILITY section, generated in R and identical on both paths, stating that the report is one realization and which figures are stable. Extensive prompt corrections, including removing fabricated references and forbidding a zone near 50 percent positive from being called majority positive.
- **`rENM.reports`** — `assemble_final_report()` substantially rewritten: `cpdf` page numbering in place of a raster overlay, letter-size normalization, optional `.docx` output, an `optional_pages` argument that lets a missing narrative page be skipped rather than aborting assembly, and a vector return value. Adds `SystemRequirements: cpdf`. The state summary table gained a boundary block and a `top_states` filter, Hot Spot % now uses a grid-measured range area as its denominator, and the mislabeled "State Area" column was renamed. Added a methods note to the report appendix, which also explains why a boundary percentage must be read together with the slope magnitudes.
