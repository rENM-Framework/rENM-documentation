...
-   After initially configuring your local [host environment](#hostenvironment) and [RStudio environment](#hostenvironment), you can set the species [banding code](#bandingcode) and step through each of the following explanations and code blocks in sequence to perform the processing steps in the **rENM workflow**.
...
This experimental module uses **generative AI (GenAI)** to support suitability trend analysis. In the first step, the function assembles an AI-ready package containing suitability trend rasters, summary tables, species metadata, and a structured prompt. In the second step, the package is submitted to to a large language model (LLM) for interpretation.
...
`OPENAI_API_KEY="sk-proj-fQpuBZIF5l4XkgB ..."`

`ANTHROPIC_API_KEY="sk-ant-api03-JuQS6V9 ..."`

`RENM_PROJECT_DIR="/Users/jschnase/rENM"`

We're currently evaluating two AI options: **ChatGPT** (model gpt-5.1) and **Claude** (model claude-sonnet-4.6). It's interesting see the differences in behavior and interpretations. Note that interacting with Claude programmatically through Antrhopic's API takes more time and costs a lot more. We'll have more on that soon ...
