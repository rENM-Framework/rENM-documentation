...
Thinning can be time-consuming. `thin_occurrences2()` processes the temporal bins in parallel and is what the pipeline uses. A sequential alternative, `thin_occurrences()`, is left commented out in the block below; it is slower but undemanding, and is worth substituting if system resources are limited or uncertain.
...
The first step in preparing environmental data is setting the spatial extent of the analysis. `find_range_extent()` derives it from the species' USGS GAP range polygon, buffered outward by 250 km, and is the method the framework uses.

The extent determines which region the predictor rasters are cropped to, and therefore which region the models draw their background from. That makes it one of the most consequential choices in the workflow. Deriving it from the GAP range keeps it independent of the occurrence records used to fit the models. An extent derived from the records themselves would move as eBird effort accumulates, which is uneven across the 1980 to 2024 window by a wide margin, and the region a model is trained against would then partly reflect where people happened to bird.

`find_range_extent()` does not use the GAP range polygon directly. It buffers the polygon outward by `buffer_km` (default 250) and takes the bounding box of the buffered result. The buffer is applied in EPSG:5070, an equal-area projection, so the margin is a true ground distance in every direction rather than a fixed number of degrees, which varies with latitude.

The buffer exists so that a moving range edge has somewhere to appear. A trend surface clipped at the historic range boundary describes only conditions inside a range delineated decades ago; it cannot show suitability improving just beyond it. The 250 km figure comes from Huang et al. (2017), who measured an average multidirectional centroid velocity of 5.89 km/yr across 57 permanent resident North American bird species. Projected across the framework's 45-year window that gives approximately 265 km, rounded down to 250 as a conservative default. The estimate is drawn from permanent residents and may not suit strongly migratory species. Run `?find_range_extent` for the full derivation and for guidance on setting a different value.

The buffer also defines two zones that later analyses report separately. The interior is the area inside the GAP range polygon. The ring is the band between that polygon and its buffered outline, the 250 km margin surrounding the range. The buffered polygon is written to `<alpha_code>/_occs/range_buffered.gpkg` so both zones can be recovered later; *Boundary trend statistics* compares trend behavior between them.
...
This step compares trend behavior inside the species' GAP range against the buffer ring surrounding it, the two zones defined in *Setting the spatial extent* above. A static range polygon cannot reveal a range edge that is moving. Strong positive trends concentrated just outside the historic boundary are the signature of an advancing leading edge, and a statistic computed over the range alone never looks there.

The function reports area, positive and negative trend percentages, and hot spot area and percentage for each zone, writing `<alpha_code>-Suitability-Trend-Boundary-Statistics.csv`. Figures are range-wide rather than per state. The ring extends into states holding none of the range, which have no counterpart row in the state-level table, and a statistic computed over a handful of cells is unstable between model realizations while the range-wide interior and ring figures reproduce closely.

Run this after the hotspot analysis above. It reads the hot spot mask that step produces, together with the buffered polygon written when the extent was set.
...
All three produce the same file, `<alpha_code>-Suitability-Trend-Analysis.docx`, which *Rendering the page* converts for inclusion in the final report. Running the whole pipeline with `rENM()` selects among them with its `ai` argument, and falls back to the coversheet if a provider call fails.
...
This step converts the DOCX produced above, whether written by a provider or by `assemble_coversheet()`, to PDF via LibreOffice for inclusion in the final report. LibreOffice must be installed before running this function (see *System Requirements*).

The function also checks the document it received. An unsubstituted placeholder stops the run, since a placeholder standing in for a figure means the page is missing data; truncated paragraphs and known prose faults produce warnings. If it stops, regenerate the narrative by re-running the submission step rather than looking for a LibreOffice problem.
...
The assembled report opens with the interpretation page produced in the previous section, followed by the eight summary pages built under *Report Generation*, and closes with two appendices: a methods note describing how the modeled extent and the boundary statistics are defined, and the MERRA variable reference. The appendices are static assets that ship with `rENM.reports` rather than products of the run, so they are controlled by the `appendix` argument rather than by `pages`. Passing `appendix = NULL` omits them. Unlike a workflow page, a missing appendix file is always an error. Page numbers are stamped on every page but the first, which carries the title block.

The opening page is the one exception to that rule. If `<alpha_code>-Suitability-Trend-Analysis.pdf` is absent, whether because a submission failed or because neither a provider nor a coversheet was run, it is omitted with a warning and the rest of the report assembles. Every other page is required, and a missing one raises an error, since that normally means an earlier step did not finish.
