...
This manual guides users through the rENM modeling and analysis workflow. Appendix C indexes every function in the framework and names the section that covers it. Detailed documentation for individual functions is available within the respective package manuals. Additional information about this work can be found in the Appendix A publications and the project's GitHub repository at: <https://github.com/rENM-Framework>.
...
If you prefer not to run an rENM analysis step by step, you can execute the full pipeline from the RStudio console with a single function call. For example: `rENM("CASP")`. Once the framework is installed and loaded, enter `?rENM` in the RStudio console for details.
...
The rENM Framework versions three things independently: the software, this manual, and the datasets. They do not advance in step, and a version number in one implies nothing about the others.

This manual describes Framework v0.2.0. A Framework version is a specific, tested combination of the seven package versions. Which combination constitutes which version is recorded in `FRAMEWORK-CHANGELOG.md` in the [rENM-documentation](https://github.com/rENM-Framework/rENM-documentation) repository, together with the Zenodo DOI for each component.

The example and extended datasets are at v0.1.0 and remain current. Framework v0.2.0 runs against them unchanged, and the download instructions below are correct as written. The `v0.1.0` in a dataset filename refers to the dataset's own release line rather than to a software version; the two coincide because both were first published together.

To reproduce results from a specific publication, install the package versions tagged for the Framework version that publication names rather than the current state of `main`. The changelog gives the tags, and the project's GitHub landing page shows the pinned installation form.

# System Requirements

The rENM Framework requires R (≥ 4.1.0) and three external software dependencies, LibreOffice, Chrome or Chromium, and cpdf, that must be installed separately before the full pipeline can run. The Framework has been developed and tested on macOS and is expected to be compatible with Linux. Windows users may experience longer run times due to limited support for parallel processing on that platform. All R package dependencies are installed automatically when you install the framework packages from GitHub (see *Installing the framework packages* below).

## R package dependencies

Two framework packages pull in heavy R dependencies worth anticipating before installation:
...
`rENM.reports::assemble_final_report()` uses cpdf (Coherent PDF Tools) to normalize every page to letter size and to stamp page numbers on the assembled report. Both operations run through cpdf, so it must be installed before assembling a final report.
...
Unlike the Chrome dependency, an absent cpdf stops report assembly rather than skipping an output. Setting `page_numbers = FALSE` does not remove the requirement, since page normalization uses cpdf regardless.
...
You only need to configure your host environment once. The examples below assume a macOS or Linux environment using the Bash shell.
...
# set the run seed; see Reproducibility under Time Series Construction
seed <- 42
set.seed(seed)
```
...
Thinning can be time-consuming. `thin_occurrences2()` processes the temporal bins in parallel. A sequential alternative, `thin_occurrences()`, is left commented out in the block below; it is slower but undemanding, and is worth substituting if system resources are limited or uncertain.
...
The first step in preparing environmental data is setting the spatial extent of the analysis. `find_range_extent()` derives it from the species' USGS GAP range polygon, buffered outward by 250 km. Appendix D gives the derivation of the 250 km figure, what the buffer does and does not affect, and the limitations of the default. Run `?find_range_extent` for the function's arguments.
...
Several stages of the analysis draw on a random number generator. Left unseeded, each draws fresh on every run, so repeating an analysis on identical inputs produces different results, by a point or two range-wide but sometimes far more for an individual state. Setting a seed fixes those draws and makes a run repeatable. It does not make any individual figure more reliable, since it selects one realization from the distribution the pipeline would otherwise sample. Appendix E covers how the seed is applied, what determinism guarantees, and the conditions under which it holds.
...
A Monte Carlo variable selection procedure identifies the highest-contributing predictors for each temporal bin before staging them, and is the recommended option for most analyses. The screener evaluates randomly selected pairs of predictors across independent model runs and keeps the variables that consistently emerge as top contributors. Screening is performed independently for each bin, so selection is informed only by the occurrence records and environmental data for that interval. `screen_by_convergence2()` uses the R package `maxnet` and has no external dependencies. See Schnase et al. (2022) for the method.

```{r}

# screen variables by convergence
...
This optional step removes covarying predictors from the staged environmental variable sets using variance inflation factor (VIF) screening. By default, a “light-touch” approach is applied using a relaxed VIF threshold (10) and a high correlation threshold (0.95), removing only strongly redundant variables while preserving alternative representations of environmental gradients.
...
This step computes the same trend sign statistics restricted to cells within the species' USGS GAP range.
...
The same `find_trend_percentages()` function used above summarizes the rate-of-change raster when passed `layer = "Suitability-Change-Trend"`. It must run after the map, since that is what produces the raster it reads. The result is a second percentages file giving the area and share of the extent where suitability trends are accelerating and decelerating. The report and the GenAI narrative both take their acceleration figures from this file.

```{r}

# summarize accelerating and decelerating proportions
rENM.analysis::find_trend_percentages(
  alpha_code = alpha_code,
  layer = "Suitability-Change-Trend")
```
...
## Boundary trend statistics

This step compares trend behavior in two zones. The interior is the area inside the species' GAP range polygon. The ring is the band between that polygon and its buffered outline, the 250 km margin surrounding the range, recovered from the buffered polygon written when the extent was set. Strong positive trends concentrated just outside the historic boundary may be the signature of an advancing leading edge, which a statistic computed over the range alone cannot show. The function reports area, positive and negative trend percentages, median and median absolute slope, and hot spot area and percentage for each zone, range-wide rather than per state. Read the percentages together with the medians: a percentage counts area above and below zero and says nothing about how large the trends are. Run it after the hotspot analysis above, whose output it reads. Appendix D describes the two zones further.

```{r}

# compare trend behavior inside the GAP range against the surrounding ring
...
This step produces the report's opening page, and there are three ways to do it.

Two use generative AI to write a structured interpretive narrative from the pipeline's quantitative outputs. Anthropic (Claude) and OpenAI (ChatGPT) are both supported and both accept the same assembled data package; the choice affects interpretation style, response time, and per-run cost. This module is experimental.

The third uses no AI. It produces a plain coversheet carrying the species name, the list of figures included in the report, the framework citation, and a timestamp, leaving the report to consist only of computed results. Skip *Assembling the AI package* and *Submitting to a provider*, and run *Building a coversheet instead*.

## API key configuration

Required for the ChatGPT and Claude paths only. Skip this if you are producing a coversheet.
...
After editing `.Renviron`, restart R or run `readRenviron("~/.Renviron")` to make the keys available in the current session. Obtain an Anthropic API key at <https://platform.claude.com/> and an OpenAI key at <https://platform.openai.com>. Enter `?submit_to_claude` or `?submit_to_chatgpt` for current model availability and pricing guidance.

## Assembling the AI package

This step and the next belong to the two provider paths.

This step collects suitability trend rasters, summary statistics, variable contribution tables, centroid displacement metrics, and species metadata from the rENM analysis pipeline and organizes them into a data bundle with a structured analytical prompt. The bundle is staged for submission to the target provider.
...
## Building a coversheet instead

If you are not using AI, this step produces the opening page directly. It writes the same `<alpha_code>-Suitability-Trend-Analysis.docx` a provider would have returned, carrying the title block, the list of figures included in the report, the framework citation, and a timestamp. No API key is needed and nothing is sent anywhere.

Run this instead of the two steps above rather than in addition to them. It writes the same filename, so running it after a successful submission overwrites the narrative with the coversheet. It is commented out by default for that reason; uncomment it and comment out the two steps above to take this path.

Run it also if a provider call failed and you want to continue. `assemble_final_report()` treats the opening page as optional and omits it when the file is absent, so a report assembled after a failed submission without this step opens directly on the suitability time series.

```{r}

# build the report's opening page without AI
# rENM.ai::assemble_coversheet(alpha_code)
```

## Rendering the page

This step converts the DOCX produced above, whether written by a provider or by `assemble_coversheet()`, to PDF via LibreOffice, which must be installed first (see *System Requirements*).

The function checks the document before converting it. An unsubstituted placeholder stops the run, because a placeholder standing in for a figure means the page is missing data. Truncated paragraphs and known prose faults only warn. A stop here is a problem with the narrative, not with LibreOffice: re-run the submission step.

```{r}

# convert the DOCX page to PDF
rENM.ai::render_ai_docx(alpha_code)
```

An AI-generated narrative is intended as a preliminary interpretation to support expert review. It should not be treated as a validated ecological conclusion.
...
The assembled report opens with the interpretation page produced in the previous section, followed by the eight summary pages built under *Report Generation*, and closes with two appendices: a methods note defining the modeled extent and the boundary statistics, and the MERRA variable reference. The appendices ship with `rENM.reports` and have their own argument; `appendix = NULL` omits them.

A missing page is an error, since that normally means an earlier step did not finish. The opening page is the exception: if `<alpha_code>-Suitability-Trend-Analysis.pdf` is absent, whether because a submission failed or because neither a provider nor a coversheet was run, it is omitted with a warning and the rest of the report assembles.

Pass an explicit `pages` argument to include a subset of pages or to change their order. The commented example below lists the default set in its standard order.
...
The rENM Framework also provides an extended dataset that includes eBird occurrence records for 18 North American bird species, the complete set of MERRA-2 and MERRAclim-2 environmental predictor variables for nine five-year bins spanning 1980–2020, long-term trend and rate-of-change maps for all variables, USGS GAP range shapefiles for the additional species, and pipeline metadata files. The part files total approximately 30 GB and unzip to 73.1 GB. Make sure sufficient disk space is available before starting the download.

Make sure your current working directory is the project directory, then run the following commands from a terminal window:
...
The `ln -sfn` command updates the `data` symbolic link to point to the extended dataset. After this step, all framework operations will use the extended dataset automatically.

## Adding new species

Three things are required to add a new species to the rENM Framework: an eBird Basic Dataset file for the species, a USGS GAP range shapefile for the species, and an entry in the species registry. The example dataset provides a useful template for all three; examine the `rENM-Framework-v0.1.0-example-data/` directory structure before proceeding.
...
The MERRA-2 and MERRAclim-2 collections included in the framework's example and extended datasets provide the best available templates for constructing a new framework-compatible variable collection. Examining those directory structures before attempting to add new data is strongly recommended. Mapping out protocols for extending the types of occurrence and environmental data accommodated by the framework is an important focus of future development.
...
Schnase, J. L., M. L. Carroll, P. M. Montesano, and V. A. Seamster. 2026. The rENM Framework: A modular system for reconstructing and analyzing long-term ecological niche dynamics. Preprint, bioRxiv. <https://doi.org/10.64898/2026.08.06.741224>. Submitted to PLOS One; review in progress.
...
| `check_species()` | Audit the species table for complete records and matching data directories | *Extending the Framework* |
...
| `set_up_run()` | Create the run directory structure for a species |
...
| `find_boundary_trend_statistics()` | Compare trend behavior inside the GAP range against the surrounding buffer ring |
...
| `assemble_coversheet()` | Build the report's opening page without calling a language model |
| `render_ai_docx()` | Convert the report's opening page from DOCX to PDF via LibreOffice |
...
| `assemble_final_report()` | Combine all pages and appendices into a single paginated PDF report |
...
# APPENDIX D - Modeled extent and boundary statistics

This appendix reproduces the methods note appended to every assembled report.

### Modeled extent

The modeled extent is a rectangular bounding box enclosing the USGS GAP range polygon for the species after that polygon has been buffered outward by 250 km, computed in a projected equal-area coordinate system (EPSG:5070, USA Contiguous Albers Equal Area Conic). The buffered polygon is species-specific. The distance is not. The same 250 km is used for every species. This keeps the boundary condition standardized and avoids setting a parameter of the analysis from centroid velocity, which the analysis itself produces.

### Derivation of the 250 km buffer

The figure derives from Huang, Sauer, and Dubayah (2017), who tracked the abundance-weighted geographic centroid of 57 permanent resident North American bird species over 44 years of Breeding Bird Survey data and found an average multidirectional centroid velocity of 5.89 km/yr. Projected across rENM's 45-year study window this gives approximately 265 km. Huang et al. report a 259 km average displacement directly, over a comparable period. The round figure of 250 km is slightly conservative against both.

Other bird range-shift estimates report only a poleward or latitudinal component of movement. The multidirectional figure was chosen because rENM computes a full two-dimensional suitability centroid, and the pilot species examined during development did not shift uniformly poleward.

### What the buffer does and does not affect

Range-based summary statistics are computed against the unbuffered GAP range polygon, not the bounding box. This covers positive and negative trend percentage, hot spot area, and hot spot percentage. The buffer and its enclosing rectangle keep the trend surface from being clipped at the historic range edge. They do not redefine what counts as range in the reported tables.

The buffer ring is the zone between the raw GAP polygon and the 250 km buffered polygon. Trend behavior there is reported separately as a boundary trend statistic. Comparing the ring against the range interior shows whether conditions just outside the historic range are improving or deteriorating relative to the occupied range. A static range polygon cannot show this.

Boundary statistics are reported range-wide rather than per state. The ring extends into states holding none of the range, which have no counterpart row in the state-level table, and a statistic computed over a handful of raster cells is unstable between model realizations, while the range-wide interior and ring figures reproduce closely.

A ring percentage near 50 is not a finding of balance. Because the percentage counts area above and below zero, a zone whose slopes hover around zero produces a figure near 50 that carries no signal at all, and it will look as precise as any other. The two median columns are what tell the cases apart. Across the twelve pilot species the ring's median absolute slope ran from 4 to 70 percent of the interior's: Loggerhead Shrike's ring is 20 percent positive at 70 percent of the interior's magnitude, a genuinely declining periphery, while Eastern Meadowlark's is 51 percent positive at a median slope of -3e-07, which over the study window is no change at all. Positivity also falls steadily from the range core to its boundary rather than being uniform inside, so the interior figure is an average over a gradient.

### Limitations

The 250 km figure is drawn from permanent resident species. Huang et al. chose residents because a resident species' abundance centroid is a cleaner climate signal than a migrant's, whose breeding and wintering ranges can shift independently. The figure may not suit strongly migratory species.

The buffer distance is a provisional default. It should be revisited as more species run through the pipeline and as taxon- or guild-specific velocity estimates become practical to compute internally.

### Reference

Huang, Q., J. R. Sauer, and R. O. Dubayah. 2017. Multidirectional abundance shifts among North American birds and the relative influence of multifaceted climate factors. *Global Change Biology* 23:3610--3622. <https://doi.org/10.1111/gcb.13683>

{{< pagebreak >}}

# APPENDIX E - Reproducibility

This appendix expands the summary under *Reproducibility* in *Time Series Construction*.

Several stages of the pipeline draw on the random number generator. Occurrence records are subsampled by `limit_record_count()`, variables are screened by `screen_by_convergence2()`, and `create_ensemble_model()` draws background points and replicate partitions. Left unseeded, each stage draws fresh on every run, and repeating an analysis on identical inputs produces different results.

Setting a seed fixes those draws. `set.seed(seed)` in the configuration block covers `limit_record_count()`, which draws on the global stream. `screen_by_convergence2()` and `create_timeseries()` take a `seed` argument of their own, because both distribute work across parallel workers and which worker receives which task is not fixed between runs; seeding inside those functions, per iteration and per year, is what makes the result independent of that scheduling. `rENM()` does the same thing internally, with a default of 42.

A fixed seed makes a run repeatable. It does not make any individual figure more reliable, since it selects one realization from the distribution the pipeline would otherwise sample.

Determinism holds within a fixed environment. Two runs with the same seed, on the same machine and the same R and package versions, produce identical output apart from embedded timestamps. Changing the R version, or the versions of the modeling packages underneath `sdm`, can change which draw a given seed produces.
